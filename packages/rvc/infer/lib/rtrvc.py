...
class RVC:
    ...
    def infer(
        ...
        feats = feats[:, :p_len, :]
        p_len = torch.tensor([p_len], dtype=torch.long, device=self.device)
        sid = torch.tensor([0], dtype=torch.long, device=self.device)
        skip_head = torch.tensor([skip_head], dtype=torch.long, device=self.device)
        return_length2 = torch.tensor([return_length2], dtype=torch.long, device=self.device)
        return_length = torch.tensor([return_length], dtype=torch.long, device=self.device)
        with torch.no_grad():
            if self.if_f0 == 1:
